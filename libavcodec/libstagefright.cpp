...
av_log(avctx, AV_LOG_ERROR, "stage init\n");
    ...
    if (!s->orig_extradata) {
av_log(avctx, AV_LOG_ERROR, "stage init  1111\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
    ...
    meta = new MetaData;
    if (meta==NULL) {
	av_log(avctx, AV_LOG_ERROR, "stage init  2222\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
...
av_log(avctx, AV_LOG_ERROR, "1111stage init  2222\n");
        ret = AVERROR(ENOMEM);
        goto fail;
    }
